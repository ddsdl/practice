...
const userDAO = {
  checkEmail: async (item, callback) => {
    ...
    try {
      ...
      if (resp[0])
        callback({status: 500, message: '사용자가 존재합니다', data: {name: resp[0].name, email: resp[0].email}});
    ...
    try {
      ...
      if (respCheck[0]) {
        callback({
          status: 500,
          message: '사용자가 존재합니다',
          data: {name: respCheck[0].name, email: respCheck[0].email},
        });
      } else {
        // 입력시 패스워드를 암호화
    ...
    try {
      ...
      bcrypt.hash(item.password, salt, async (error, hash) => {
        if (error) callback({status: 500, message: '암호화 실패', error: error});
          ...
          const [resp] = await conn.query(sql.signup, [hash, item.email]);
